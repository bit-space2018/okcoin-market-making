...
class AccountAPI(Client):
    ...
    # coin withdraw
    def coin_withdraw(self, currency, amount, target, withdraw_address, trade_pwd, fee):
        params = {'currency': currency, 'amount': amount, 'target': target, 'withdraw_address': withdraw_address, 'trade_pwd': trade_pwd, 'fee': fee}
        return self._request_with_params(POST, COIN_WITHDRAW, params)

    # query the fee of coin withdraw
    def get_coin_fee(self, symbol=''):
        if symbol:
            return self._request_with_params(GET, COIN_FEE, {'currency': symbol})
        else:
            return self._request_without_params(GET, COIN_FEE)
    ...
    # coin transfer
    def coin_transfer(self, currency, amount, account_from, account_to, sub_account='', product_id=''):
        params = {'currency': currency, 'amount': amount, 'from': account_from, 'to': account_to, 'sub_account': sub_account, 'product_id': product_id}
